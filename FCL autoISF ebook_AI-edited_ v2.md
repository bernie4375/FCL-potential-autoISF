...
<p class="cover-eyebrow">The Practical Guide to</p>
<h1 class="cover-title">Full Closed Loop</h1>
<p class="cover-dinkus">&#42; &#42; &#42;</p>
<p class="cover-tagline">And a Comprehensive Exploration of autoISF</p>
</div>
<div class="cover-foot">
<p class="cover-author">Authored by Bernd Herpichboehm, PhD</p>
<p class="cover-credit">Restructured and edited for clarity</p>
</div>
</div>

<!-- ===================== AUTHOR LEAF ===================== -->

<div class="author-leaf">

# Author

<p class="chapter-subtitle">Bernd Herpichboehm, PhD*</p>
...
<p class="chapter-subtitle">Please Read This First</p>
...
1. **Do not “learn by doing” with dangerous tools.** Pressing buttons without understanding them, with a DIY closed-loop insulin system, is a serious risk.
1. **Start disconnected.** In your early testing phase, run the system as a dummy — not attached to your body — in parallel with your current glucose management, until you understand its behavior.
1. **Stay connected to the community.** Share experiences. The only way this work improves is through users who keep talking to each other and to the developers.

-----

# Preface

<p class="chapter-subtitle">How to Use This Book</p>
...
<p class="chapter-subtitle">What Is autoISF Full Closed Loop?</p>
...
1. **FCL with autoISF.** The subject of this book. More sophisticated, more work to set up, capable of higher time-in-range and more daily “freedom” from intervention.
...
<p class="chapter-subtitle">The Journey to Full Closed Loop</p>
...
1. Widen the max allowed ISF amplification (autoISF_max).
1. Nudge up the SMB delivery ratio.
1. **Install your iob threshold (iobTH)** — this is your most important new safety setting.
1. Configure a handful of other AAPS preferences (Autosens off, exercise settings, odd/even target logic, etc.).
...
1. **Tap the violet FCL loop icon** on the AAPS home screen and pick the green HCL icon (if your version supports it).
...
<p class="chapter-subtitle">Prerequisites for Full Closed Loop</p>
...
1. **autoISF multiplies your profile_ISF.** Every tuning parameter you will set in Chapter 4 is a *factor on your profile ISF for that hour.* If your profile ISF is wrong by 20%, every bgAccel, pp, and dura weight you tune will be calibrated to compensate — and will be wrong for the same reason.
1. **New parameters stack on old ones.** If you tune new parameters on top of an unfixed old profile, you create a system of balanced errors. This can work in individual scenarios but creates an unstable system overall, and is nearly impossible to untangle later.
...
1. **What smoothing, if any, is applied?** See <https://androidaps.readthedocs.io/en/latest/Usage/Smoothing-Blood-Glucose-Data.html>
1. **Does your CGM produce artefacts?** Review several days of your own data, looking for sudden jumps that the loop could mistake for a meal. Compression lows at night are a particularly common culprit.
1. **If your CGM requires calibrations**, calibration events often produce jumps. You will need a protocol for pausing FCL around calibrations.
...
1. **Know how to shut off SMBs quickly.** Setting an odd-numbered temporary target blocks all SMBs while active. This is your emergency brake. Use it for:
- Anti-hypo snacks
- Unusual situations where you do not want the loop to react aggressively
- Any moment when something feels off
1. **Practice exiting FCL.** In your setup weeks, deliberately switch from FCL to HCL and back several times, so you know the motions by heart before you ever need them in a hurry. See Chapter 10 (Troubleshooting, §10.1) and Chapter 5 (§5.6).
...
<p class="chapter-subtitle">How autoISF Works</p>
...
1. **Classifies the phase** — is this an acceleration? A steep linear rise? A deceleration? A plateau?
1. **Calculates an adapted ISF** (called `sens` in the AUTO ISF tab) by applying modulation factors to your profile_ISF for that hour.
1. **Asks the standard oref(1) UAM loop what insulin is needed** given that adapted ISF.
1. **Delivers an SMB and/or adjusts temp basal**, capped by several safety limits.
...
1. Avoid sweet drinks at meal start.
1. Review logs to see how often this triggers for you, and adjust behavior.
...
1. **Extra graphs below the AAPS home screen glucose chart.** These can display iobTH, the autoISF factor contributions over time, the fitted parabola, and more. Enable them from the chart options.
1. **The Emulator** (Chapter 11–14). Replays your logfiles on a PC or phone and shows loop decisions in tabular form, plus lets you ask “what if I had used setting X instead?”
...
<p class="chapter-subtitle">General FCL Settings</p>
...
1. ☐ **Autosens: OFF.** autoISF is incompatible with Autosens in FCL mode. You’ll see a warning when Autosens is on without carb inputs.
1. ☐ **Autotune: do not use.** Likewise, **dynamicISF (in iAPS/Trio): OFF**. And **dynamic CR, sigmoid: OFF**.
1. ☐ **SMB frequency:** 3 minutes (standard), 1 minute for Libre 3.
1. ☐ **“High TT raises sensitivity”: ON.** Required for the exercise mode math.
1. ☐ **“Low TT lowers sensitivity”: OFF** at first. Turn ON later, once you understand how aggressive your loop can go.
1. ☐ **“Resistance lowers target” and “Sensitivity raises target”: OFF**. These interact poorly with autoISF — especially the even/odd target logic.
1. ☐ **Half-basal exercise target: 160 mg/dL** placeholder. You’ll tune this in Chapter 6.
1. ☐ **“Activity modifies sensitivity”: OFF** until Chapter 6. (You can turn it on sooner if you’re already sure about your step-count tuning.)
1. ☐ **Advanced → “Always short avg delta”: OFF** unless your CGM is jittery. Smoothing costs you early detection time.
1. ☐ **Smoothing** (AAPS Configuration Builder → Smoothing): None is preferred for excellent CGMs. Average is next best. Exponential smooths nicely but hides the early rise that FCL depends on. G7 users typically need exponential smoothing.
1. ☐ **Safety multipliers** (Preferences → OpenAPS SMB → Advanced): roughly **double** the HCL defaults, so the loop can do up to 500% TBR.
1. ☐ **`iob_threshold_percent`**: enter the value from §3.4.
1. ☐ **Password-protect Preferences** with a short password, to avoid accidental changes while scrolling.
1. ☐ **Disable bottom buttons** (Preferences → Overview → Buttons: Insulin, Calculator, etc. all OFF). Especially important if a child uses the phone, or if it might be temporarily out of your control. The insulin button allows a direct insulin shot, which is dangerous.
1. ☐ **Alarms:** review your past outlier data and set xDrip / AAPS alarms that would have caught those issues.
1. ⚠️ **autoISF settings**: do not yet enable “Enable ISF adaptation by glucose behavior.” You will do this when you start Chapter 4.
...
<p class="chapter-subtitle">Meals: Setting Your ISF Weights</p>
...
1. **Start with typical meals from your own spectrum.** Not extremes. Not novelty meals. Not sweet drinks at meal start.
1. **Iterate on 2–3 different meals, not one.** Settings optimized for a single meal will rarely work for the rest. (Case Study 8.2 is the negative example.)
1. **Do not give a bolus** during this phase. Any user bolus distorts the glucose curve that autoISF is reading, and invalidates your tuning.
1. **Start narrow.** Many users begin with only one meal per day (e.g., dinners), with an Automation that turns autoISF off for the other 20 hours. Expand once one meal works.
1. **Do not copy another person’s numbers.** Every example in this chapter is illustrative only.
...
1. **Open the AUTO ISF tab** at key moments to see exactly what the loop decided and why.
...
1. Only tune on days you know are good CGM days.
1. Live with average CGM quality and accept weaker/later SMBs and narrower overall dynamic range.
...
1. **Lower dura_ISF_weight** in small steps.
1. **Add an Automation** to cap dura contributions once iob exceeds a threshold — advanced, see Chapter 5.
...
1. Note where the loop did well and where it fell short (too-high peak? late hypo? over-reacted to a snack?).
1. Pick the *first* thing to change — usually the earliest-acting weight with a problem.
1. Make a ~10–20% change.
1. Repeat for a few more meals before changing again.
...
1. **Keep a small snack on hand** for when the insulin tail drops you after a high peak.
1. **Manually intervene** — a brief odd TT for a snack situation, for example.
1. **Define a User Action Automation** that gives that meal type its own settings. See §5.4.
1. **Fall back to HCL** for those specific meals.
...
> 1. **iobTH_percent** — revisit after bgAccel is stable.
> 1. **pp_ISF_weight** — second most impactful.
> 1. **dura_ISF_weight** — mostly for low-carb or fatty meals.
> 1. **bgBrake_ISF_weight** — leave at half of bgAccel unless low-carb is a big part of your diet.
> 1. **bg_ISF_weight** — leave at 0 in FCL.
...
<p class="chapter-subtitle">Modulating Loop Aggressiveness</p>
...
1. You will forget to change it back.
...
1. Set **User action** = ticked. Configure a time window (e.g., 2 PM – 6 PM) when the button should appear.
1. Under **Actions**:
...
1. **Temporarily set `bgAccel_ISF_weight` to 0.** Keeps dura_ISF and the rest working, but disables the aggressive first-SMB behavior.
1. **Tap the loop icon** (violet circle → green circle) if your AAPS version includes this UI feature. This will switch the Overview buttons back on too.
...
1. **The `ai: %` indicator** under Autosens% on the home screen — shows whether autoISF is adapting right now.
1. **The AUTO ISF tab** — full detail on the most recent loop decision, including:
...
1. **Optional extra graphs** below the main AAPS glucose chart — iobTH corridor, ISF factor contributions, fitted parabola.
...
<p class="chapter-subtitle">Exercise and Activity</p>
...
1. **Temporary %profile switch** — scales everything down.
1. **The “DIY cockpit” + Automation sequence** — pre-programs the whole thing into one button press.
...
1. For each kind of exercise you do regularly, find a TT that produces the right amount of softening. Mnemonic: choose a memorable number so “TT = 140 = my jogging TT” is easy to remember.
1. Iterate across a few sessions.
...
1. After iobTH is exceeded (approximately when the first meal SMBs finish): **set an elevated TT** and the exercise mode.
...
1. On a few typical days (some walking-heavy, some sedentary), check the AUTO ISF tab to see what it’s doing.
1. If it over-softens during light walks, reduce the activity factor.
1. If it doesn’t soften enough, increase the activity factor.
1. Same for inactivity.
...
<p class="chapter-subtitle">Advanced HCL with Meal Announcement</p>
...
1. **Meal management tuning:** check that your ISFs handle the rising bg after your meal bolus loses power.
1. **With correct ISFs, expand SMB sizes to 120 minutes of basal.**
1. **Handle fatty-meal insulin resistance.** Two options:
...
<p class="chapter-subtitle">Other Avenues to Full Closed Loop</p>
...
<p class="chapter-subtitle">Performance Monitoring</p>
...
1. **Nightscout Reporter** (or equivalent) — weekly time-in-range, hypo counts, daily curves. Useful for tracking trend direction.
1. **AAPS home screen** — real-time situational awareness during the day. Don’t stare at it, but glance.
...
1. **Is a specific meal or snack habit to blame?** Are there “bad” meals and “good” meals?
1. **Is there a pattern with exercise or hormonal/disease cycles?** Period week? A strength training day you added?
1. **Does it correlate with infusion site age, or CGM day 1?** “Bad” cannulas and new sensors can masquerade as a tuning problem.
1. **Have you changed anything recently?** A new Automation, a profile tweak, a diet change? Start by undoing it.
...
<p class="chapter-subtitle">Troubleshooting</p>
...
1. **Tap the loop icon** (violet FCL → green HCL) if your AAPS version supports this UI. Automatically re-enables the overview buttons.
...
1. **Reduce `bgAccel_ISF_weight`** if early SMBs are too big.
1. **Identify the meal types** where this happens — often low-fibre, low-fat, low-protein meals where the insulin tail outlasts the carb absorption.
1. **Take a small snack** proactively (5–10g) when the loop warns of an imminent hypo. The loop’s carb recommendation is often inflated — you usually need much less than it suggests, because the loop doesn’t know about fat/protein still digesting.
1. **Adjust your diet** — add more fibre, protein, or fat to balance the carb absorption curve.
1. **Lower iobTH** — if hypos reliably trace to too much iob in the rise phase.
1. **Accept the tradeoff.** Sometimes a small snack is genuinely the right answer — “better a snack than a hypo” — especially for weight-conscious users who can reduce intake at the meal itself to compensate.
...
1. **Fall back to HCL** — either vanilla AAPS HCL, or HCL with just dura_ISF active for fatty-meal management.
1. **Switch methods** — try AAPS Master FCL with Automations (Chapter 8, §8.1), which is simpler.
1. **Temporarily switch to a commercial system** — wait for better tools, then re-evaluate.
...
1. **Don’t optimize for single events.** The goal is “good-enough through nearly all scenarios.” (See Case Study 8.2.)
1. **Keep it simple.** Resist the latest trick until you understand how it affects your existing balance.
1. **Use FCL selectively.** It’s fine to run FCL only for some meals, or only on certain days. Don’t force the loop into situations where you already know it will struggle.
1. **Stay connected to the community.** The Discord channels are where problems get diagnosed fastest: <https://discord.gg/tamvhh57Xs>
1. **Keep perspective on %TIR expectations.** Many non-looping T1Ds are happy above 60% TIR. A study of 16 AAPS users achieved ~80% TIR on a simpler FCL with no meal announcement. These are reasonable reference points. You don’t have to chase 95%.
...
<p class="chapter-subtitle">The Emulator on Your PC</p>
...
1. Keep a copy at the top of your `Emulator_Studies/` folder. For each new study, copy it into the Study_n folder.
...
1. **VDF file** — for a no-change run, the `noChange.vdf` in that same Study_n folder.
1. **Logfiles** — browse to any logfile from the day you want to analyze. Then:
...
<p class="chapter-subtitle">The Emulator on Your Phone</p>
...
1. **Real-time “what-if” announcements** — spoken aloud by the phone, when a changed setting would have produced a different SMB.
...
1. Copy it to the phone’s AAPS logs folder.
1. Select it in the running QPython emulator (via a menu action — see the dev docs).
1. The emulator now runs your change in parallel with the real loop, for each new CGM value.
...
1. Decide whether to follow the suggestion by **manually adding a bolus** of the suggested size. (You’ll need to temporarily re-enable the insulin button in AAPS Preferences → Overview → Buttons → Insulin = ON during test phases.)
1. Watch what happens over the next hour.
...
1. **Use a “silent” custom `.config`** with very narrow time windows for announcements, and switch to it via the emulator’s menu.
1. **Silence the phone** (mute + do not disturb) — blunt, shuts off other alerts too.
1. **Kill QPython** (force-stop the app) — emulation stops entirely. No tabular data for that period.
...
<strong>_weight</strong> → see `ISF_weight`.
